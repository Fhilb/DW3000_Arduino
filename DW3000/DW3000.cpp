...
int* DW3000::getBase(int hex_num)
{
    return hexToBin(hex_num, 5);
}

int* DW3000::getSub(int hex_num)
{
    return hexToBin(hex_num, 7);
}

int* DW3000::hexToBin(int hex_num, int bit_size)
{
...
int DW3000::getAnchorID() {
	return anchor_id;
}


uint32_t DW3000::sendBytes(int b[], int lenB, int recLen) { //WORKING
...
uint32_t DW3000::readOrWriteFullAddress(int base[], int base_len, int sub[], int sub_len, int data[], int data_len, int readWriteBit) {
    int fill_base_len = 5;
...
uint32_t DW3000::read(int base, int sub) {
    int* _base = DW3000::getBase(base);
    int* _sub = DW3000::getSub(sub);
    int t[] = {0};
...
uint32_t DW3000::write(int base, int sub, int *data, int data_len) {
    int* _base = DW3000::getBase(base);
    int* _sub = DW3000::getSub(sub);
...
void DW3000::init() {
    ...
    DW3000::write(0x0, 0x3C, data, 6); //Set IRQ for successful received data frame

    int data2[] = { 0x03, 0x1C };
    DW3000::write(0x0, 0x24, data2, 2); //Frame length setup for Transmission  

    int data11[] = { 0x00,0x9 };
    DW3000::write(0xA, 0x0, data11, 3); //AON_DIG_CFG register setup; sets up auto-rx calibration and on-wakeup GO2IDLE

    /*
     * Set RX and TX config
     */
    int data15[] = { 0x10,0x00,0x02,0x40 }; //DGC_CFG0
    DW3000::write(0x3, 0x1C, data15, 4);

    int data16[] = { 0x1b,0x6d,0xa4,0x89 }; //DGC_CFG1
    DW3000::write(0x3, 0x20, data16, 4);

    int data17[] = { 0x00,0x01,0xC0,0xFD }; //DGC_LUT_0
    DW3000::write(0x3, 0x38, data17, 4);

    int data18[] = { 0x00,0x01,0xC4,0x3E }; //DGC_LUT_1
    DW3000::write(0x3, 0x3C, data18, 4);

    int data19[] = { 0x00,0x01,0xC6,0xBE }; //DGC_LUT_2
    DW3000::write(0x3, 0x40, data19, 4);

    int data20[] = { 0x00,0x01,0xC7,0x7E }; //DGC_LUT_3
    DW3000::write(0x3, 0x44, data20, 4);

    int data21[] = { 0x00,0x01,0xCF,0x36 }; //DGC_LUT_4
    DW3000::write(0x3, 0x48, data21, 4);

    int data22[] = { 0x00,0x01,0xCF,0xB5 }; //DGC_LUT_5
    DW3000::write(0x3, 0x4C, data22, 4);

    int data23[] = { 0x00,0x01,0xCF,0xF5 }; //DGC_LUT_6
    DW3000::write(0x3, 0x50, data23, 4);

    //SET PAC TO 32 (0x00) reg:06:00 bits:1-0, bit 4 to 0 (00001100) (0xC)
    int data25[] = { 0xE };
    DW3000::write(0x6, 0x0, data25, 1);

    //SET PREAMBLE CODE (RX_PCODE, TX_PCODE) TO 10 (reg:01:14) //Standard SFD Type is 11 (data: 0x56, 0x5), trying 00 (0x50, 0x5)
    int data26[] = { 0xBE, 0x3 };
    DW3000::write(0x1, 0x14, data26, 2);
     ...
     */
    int data3[] = { 0xF5,0xE4 };
    DW3000::write(0x3, 0x18, data3, 2); //THR_64 value set to 0x32

    int data5[] = { 0x2 };
    DW3000::write(0x4, 0xC, data5, 1); //COMP_DLY to 0x2

    int data6[] = { 0x14 };
    DW3000::write(0x7, 0x48, data6, 1); //LDO_RLOAD to 0x14

    int data7[] = { 0x0E };
    DW3000::write(0x7, 0x1A, data7, 1); //RF_TX_CTRL_1 to 0x0E

    int data8[] = { 0x34,0x11,0x07,0x1C };
    DW3000::write(0x7, 0x1C, data8, 4); //RF_TX_CTRL_2 to 0x1C071134 (due to channel 5, else (9) to 0x1C010034)

    int data9[] = { 0x3C,0x1F };
    DW3000::write(0x9, 0x0, data9, 2); //PLL_CFG to 0x1F3C (due to channel 5, else (9) to 0x0F3C)

    int data10[] = { 0x21 };
    DW3000::write(0x9, 0x8, data10, 1); //PLL_CFG_LD to 0x8 (Documenation says 0x81, doesn't fit the 6bit register tho)

    int data12[] = { 0x0, 0x8, 0x1 };
    DW3000::write(0xA, 0x0, data12, 3); //Auto antenna calibration on startup enable (ONW_PGFCAL)
    ...
    delay(50);
    for (int i = 0; i < 5; i++) {
        uint32_t h = DW3000::read(0x4, 0x20); //Read antenna calibration //RX_CAL_STS => Status bit, if high antenna cal was successful
        if (h > 0) {
            break;
        }
        int data29[] = { 0x5, 0x10 };
        DW3000::write(0x7, 0x48, data29, 2); //Old values in register: 10000000000000011100
        int data30[] = { 0x1 };
        DW3000::write(0x4, 0xE, data30, 1);
        if (i < 4) {
            Serial.println("Failed auto calibration of antenna. Trying again in 50ms.");
            DW3000::write(0x4, 0xC, data13, 1);
            delay(50);
        }
        else {
    ...
    DW3000::write(0x4, 0xC, data14, 1); //reset to normal operation
...
void DW3000::readInit() {
    int shCmd[] = { 0 };
    writeShortCommand(shCmd, 1);
    Serial.println("\n+++ DecaWave DW3000 Test +++\n");
    Serial.println("IRQ:");
    DW3000::read(0x0, 0x3C); //Set IRQ for successful received data frame
    Serial.println("Frame length:");
    DW3000::read(0x0, 0x24); //Frame length setup for Transmission  
    Serial.println("AON_DIG_CFG Register:");
    DW3000::read(0xA, 0x0); //AON_DIG_CFG register setup; sets up auto-rx calibration and on-wakeup GO2IDLE
    ...
    DW3000::read(0x3, 0x1C);//DGC_CFG0
    Serial.println("DGC_CFG1:");
    DW3000::read(0x3, 0x20);//DGC_CFG1
    Serial.println("DGC_LUT_0:");
    DW3000::read(0x3, 0x38);//DGC_LUT_0
    Serial.println("DGC_LUT_1:");
    DW3000::read(0x3, 0x3C);//DGC_LUT_1
    Serial.println("DGC_LUT_2:");
    DW3000::read(0x3, 0x40);//DGC_LUT_2
    Serial.println("DGC_LUT_3:");
    DW3000::read(0x3, 0x44);//DGC_LUT_3
    Serial.println("DGC_LUT_4:");
    DW3000::read(0x3, 0x48);//DGC_LUT_4
    Serial.println("DGC_LUT_5:");
    DW3000::read(0x3, 0x4C);//DGC_LUT_5
    Serial.println("DGC_LUT_6:");
    DW3000::read(0x3, 0x50);//DGC_LUT_6
    Serial.println("PAC:");
    //SET PAC TO 32 (0x00) reg:06:00 bits:1-0, bit 4 to 0 (00001100) (0xC)
    DW3000::read(0x6, 0x0);
    Serial.println("Preamble Code:");
    //SET PREAMBLE CODE (RX_PCODE, TX_PCODE) TO 10 (reg:01:14) //Standard SFD Type is 11 (data: 0x56, 0x5), trying 00 (0x50, 0x5)
    DW3000::read(0x1, 0x14);
    ...
    DW3000::read(0x3, 0x18); //THR_64 value set to 0x32
    Serial.println("COMP_DLY:");
    DW3000::read(0x4, 0xC); //COMP_DLY to 0x2
    Serial.println("LDO_RLOAD:");
    DW3000::read(0x7, 0x48); //LDO_RLOAD to 0x14
    Serial.println("RF_TX_CTRL_1:");
    DW3000::read(0x7, 0x1A); //RF_TX_CTRL_1 to 0x0E
    Serial.println("RF_TX_CTRL_2:");
    DW3000::read(0x7, 0x1C); //RF_TX_CTRL_2 to 0x1C071134 (due to channel 5, else (9) to 0x1C010034)
    Serial.println("PLL_CFG:");
    DW3000::read(0x9, 0x0); //PLL_CFG to 0x1F3C (due to channel 5, else (9) to 0x0F3C)
    Serial.println("PFF_CFG_LD:");
    DW3000::read(0x9, 0x8); //PLL_CFG_LD to 0x8 (Documenation says 0x81, doesn't fit the 6bit register tho)
    Serial.println("Auto antenna config enable:");
    DW3000::read(0xA, 0x0); //Auto antenna calibration on startup enable (ONW_PGFCAL)
    Serial.println("antenna calibration result:");
    uint32_t h = DW3000::read(0x4, 0x20); //Read antenna calibration //RX_CAL_STS => Status bit, if high antenna cal was successful

}

void DW3000::writeShortCommand(int cmd[], int cmd_len) {
